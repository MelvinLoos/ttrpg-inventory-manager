...
describe('inventoryStore', () => {
  ...
  it('autoAddItem adds an item when space exists', () => {
    ...
    const added = store.autoAddItem({ id: 'test', name: 'Test', weight: 1, slotCost: 1 })
  ...
  it('removeItem removes by instanceId', () => {
    const store = useInventoryStore()
    store.inventory = [{ id: 'x', name: 'X', weight: 1, slotCost: 1, instanceId: 'abc', position: 0 }]
  ...
  it('updatePosition moves item when valid and rejects when blocked', () => {
    const store = useInventoryStore()
    store.inventory = [{ id: 'big', name: 'Big', weight: 6, slotCost: 6, instanceId: 'big1', position: 0 }]
    // try to move to position that fits
    const ok = store.updatePosition('big1', 1)
    expect(ok).toBe(true)
    // add blocking item that prevents moving to 2
    store.inventory.push({ id: 'block', name: 'Block', weight: 3, slotCost: 3, instanceId: 'b1', position: 3 })
  ...
  it('setDraggingItem sets and clears draggingItem', () => {
    const store = useInventoryStore()
    store.setDraggingItem({ id: 'foo', name: 'Foo', weight: 1, slotCost: 1 })
